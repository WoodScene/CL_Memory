import os
os.environ['TRANSFORMERS_CACHE'] = '/workspace/CL_Memory/cache'
import sys
...
AutoConfig.default_cache_dir = '/workspace/CL_Memory/cache'
"""
Unused imports:
...
def train(
    ...
    output_dir = os.path.join(output_path, model_name + "_cl_dataset_id_"+str(dataset_id), str(task_id)+"-"+dataset_order[task_id])
    ...
    if task_id == 0:
        ...
        last_checkpoint_dir = os.path.join(output_path, model_name + "_cl_dataset_id_"+str(dataset_id), str(task_id-1)+"-"+last_service_name)
    ...
    # cl training 只需简单合并两个数据集去训练即可
    current_data = load_current_task_data(dataset_id, task_id, data_dir, cache_dir, model_name)
    if task_id > 0: # task_id是从0开始
        ...
        data = concatenate_datasets([current_data, memory_data])
    else:
        memory_data = None
        data = current_data
