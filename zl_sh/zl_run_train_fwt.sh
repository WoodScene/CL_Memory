...
export CUDA_VISIBLE_DEVICES=4   # 只用 1 张卡

# ==== WandB 离线配置 ====
export WANDB_MODE=offline                    # 离线模式，不联网、不弹登录
export WANDB_PROJECT=fwt      # 项目名
export WANDB_API_KEY="dummy_offline_key"     # 给个非空字符串即可

# ==== WandB 日志根目录 ====
export WANDB_DIR="/workspace/CL_exp_zl/wandblog/FWT_0d6B_exp_data_id_78_512"  # ✅ 你想让 wandb 写到哪，就改这里
...
do
    ...
    do
        ...
        torchrun \
            ...
            --dataset_id ${data_id} \
            --task_id ${ORDERR} \
            --output_path "/workspace/CL_exp_zl/checkpoints/exp_1_0d6B_512/" \
